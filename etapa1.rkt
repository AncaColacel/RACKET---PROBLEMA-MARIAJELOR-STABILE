...
(define (get-men mpref)
  (if (null? mpref)
      '()
      (append (list(car (car mpref))) (get-men (rest mpref)))
      )
  )
...
(define (get-women wpref)
  (get-women-iter wpref '())
  )
(define (get-women-iter L R)
  (if (null? L)
      (reverse R)
      (get-women-iter (rest L) (append(list(car(car L))) R))
      )
  )
...
(define (get-partner engagements person)
  (if (null? engagements)
      #f
  (if (equal? (car(car engagements)) person)
              (cdr(car engagements))
